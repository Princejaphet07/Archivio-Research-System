    ...
    try {
      ...
      if (error.code === 'auth/network-request-failed') {
        fireAlert({
          icon: 'error',
          title: 'Network Error',
          text: 'Network connection issue. Please check your internet connection and try again.'
        });
        return;
      }
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full font-serif bg-[#24050f] md:bg-[#faf7f2] dark:bg-[#0d090b]">
      ...
      <div className="w-full md:w-[42%] lg:w-[40%] bg-[#24050f] text-white flex flex-col justify-between p-4 sm:p-6 md:p-8 lg:p-12 relative z-20 shadow-xl md:shadow-[15px_0_30px_-5px_rgba(0,0,0,0.6)] shrink-0">
        ...
        <div className="hidden md:grid grid-cols-3 gap-3 bg-black/20 backdrop-blur-sm border border-white/10 p-3.5 rounded-xl text-center relative z-30 mt-6">
          <div>
            <p className="text-base lg:text-lg font-bold text-amber-200">{stats.papers}</p>
            <p className="text-[10px] uppercase tracking-wider text-stone-400 mt-0.5">Papers</p>
          </div>
          <div className="border-x border-white/10">
            <p className="text-base lg:text-lg font-bold text-amber-200">{stats.authors}</p>
            <p className="text-[10px] uppercase tracking-wider text-stone-400 mt-0.5">Authors</p>
          </div>
          <div>
            <p className="text-base lg:text-lg font-bold text-amber-200">{stats.programs}</p>
            <p className="text-[10px] uppercase tracking-wider text-stone-400 mt-0.5">Program</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="absolute inset-0 z-0 pointer-events-none opacity-0 dark:opacity-90 transition-opacity duration-300 bg-[#0d090b]" />

        {/* Ambient warm radial glow in dark mode */}
        <div className="absolute inset-0 z-0 pointer-events-none opacity-0 dark:opacity-100 transition-opacity duration-500 bg-[radial-gradient(circle_at_center,rgba(122,32,57,0.25)_0%,transparent_70%)]" />
        ...
        <div className="bg-white/95 dark:bg-[#1c1518]/95 backdrop-blur-xl p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-[0_10px_35px_rgba(0,0,0,0.1)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] max-w-[420px] w-full border border-stone-200/90 dark:border-white/15 flex flex-col items-center transition-all my-auto relative z-10">
          ...
          <div className="flex bg-stone-200/70 dark:bg-black/50 p-1 rounded-full mb-4 w-full max-w-[210px] relative border border-stone-300/60 dark:border-white/10">
            ...
            <button 
              ...
              className={`flex-1 text-xs font-bold py-1.5 rounded-full relative z-10 transition-colors ${isLogin ? 'text-[#7a2039] dark:text-white' : 'text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200'}`}
            >
              Sign In
            </button>
            <button 
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-stone-800 dark:text-[#f3e5ab] tracking-wide mb-1 font-serif">
            {isLogin ? 'Welcome Back' : 'Create an Account'}
          </h3>
          <p className="text-xs text-stone-500 dark:text-stone-300 text-center mb-4 max-w-[310px] leading-relaxed">
          ...
          <form onSubmit={handleEmailAuth} className="w-full space-y-3 mb-2 font-sans">
            {!isLogin && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    required={!isLogin}
                    className="w-full pl-9 pr-3.5 py-2 bg-white/70 dark:bg-black/40 border border-stone-300 dark:border-white/15 rounded-lg outline-none focus:border-[#7a2039] dark:focus:border-[#f3e5ab] focus:ring-2 focus:ring-[#7a2039]/10 dark:focus:ring-[#f3e5ab]/20 text-xs sm:text-sm text-stone-900 dark:text-stone-100 transition-all placeholder-stone-400 dark:placeholder-stone-500 dark:[&:-webkit-autofill]:[transition:background-color_5000s_ease-in-out_0s] dark:[&:-webkit-autofill]:[-webkit-text-fill-color:#f5f5f5]"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-9 py-2 bg-white/70 dark:bg-black/40 border border-stone-300 dark:border-white/15 rounded-lg outline-none focus:border-[#7a2039] dark:focus:border-[#f3e5ab] focus:ring-2 focus:ring-[#7a2039]/10 dark:focus:ring-[#f3e5ab]/20 text-xs sm:text-sm text-stone-900 dark:text-stone-100 transition-all placeholder-stone-400 dark:placeholder-stone-500 select-none dark:[&:-webkit-autofill]:[transition:background-color_5000s_ease-in-out_0s] dark:[&:-webkit-autofill]:[-webkit-text-fill-color:#f5f5f5]"
                  placeholder="••••••••••••"
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 dark:text-stone-400 dark:hover:text-[#f3e5ab] transition-colors cursor-pointer" title="Toggle Password Visibility">
                  {showPassword ? (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="••••••••••••"
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 dark:text-stone-400 dark:hover:text-[#f3e5ab] transition-colors cursor-pointer" title="Toggle Confirm Password Visibility">
                    {showConfirmPassword ? (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" /></svg>
                    ) : (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
            ...
            {!isLogin && (
              <div className="flex items-start gap-2.5 mt-1.5 mb-1.5">
                <input 
                  type="checkbox" 
                  id="terms"
                  checked={agreedToTerms}
                  onChange={(e) => setAgreedToTerms(e.target.checked)}
                  className="mt-0.5 cursor-pointer w-3.5 h-3.5 accent-[#7a2039] dark:accent-[#f3e5ab]"
                />
                <label htmlFor="terms" className="text-[11px] text-stone-600 dark:text-stone-300 leading-snug">
            ...
            <ReCaptcha
              ...
              className="pt-1 pb-1 scale-90 sm:scale-95 origin-center"
            />

            <button 
              type="submit" 
              disabled={loading || (!isLogin && !agreedToTerms)}
              className={`w-full py-2.5 bg-gradient-to-r from-[#7a2039] to-[#541223] hover:from-[#8b2440] hover:to-[#631529] text-white rounded-lg text-xs sm:text-sm font-bold tracking-wider uppercase shadow-md shadow-[#7a2039]/20 hover:shadow-[#7a2039]/40 transition-all cursor-pointer ${loading || (!isLogin && !agreedToTerms) ? 'opacity-60 cursor-not-allowed' : 'active:scale-[0.99]'}`}
            >
              {loading ? 'Authenticating...' : (isLogin ? 'Sign In' : 'Sign Up')}
            </button>
          </form>

          <div className="relative flex items-center justify-center w-full mt-3.5 mb-2.5">
          ...
          <button 
            ...
            className={`w-full py-2 px-3.5 flex items-center justify-center gap-2.5 bg-white hover:bg-stone-50 dark:bg-white/5 dark:hover:bg-white/10 border border-stone-300 dark:border-white/15 rounded-lg font-sans font-medium text-xs sm:text-sm text-stone-700 dark:text-stone-200 transition-all shadow-sm ${isGoogleLoading || loading ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
          >
            {isGoogleLoading ? (
              <span className="flex items-center gap-2 text-stone-600 dark:text-stone-300">
                <svg className="animate-spin h-4 w-4 text-[#7a2039] dark:text-[#f3e5ab]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <>
                ...
                <span className="truncate">Continue with Google (@phinmaed.com)</span>
              </>
            )}
          </button>
