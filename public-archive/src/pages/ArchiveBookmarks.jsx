...
function ArchiveBookmarks() {
  ...
  useEffect(() => {
    const fetchPaperDetails = async (paperIds) => {
      ...
      const validPapers = results.filter(p => p !== null);
      setBookmarkedPapers(validPapers);
      setIsOfflineLoaded(false);
      setLoading(false);

      if (currentUser?.uid && validPapers.length > 0) {
        try {
          localStorage.setItem('archivio_cached_bookmarks_' + currentUser.uid, JSON.stringify(validPapers));
        } catch (_) {}
      }
    };
    ...
    // Attempt offline cache instant load first if offline or on initial mount
    const cached = localStorage.getItem('archivio_cached_bookmarks_' + currentUser.uid);
    if (cached) {
      try {
    ...
    const unsub = onSnapshot(
      doc(db, 'user_bookmarks', currentUser.uid),
      (docSnap) => {
        if (docSnap.exists()) {
      ...
      (error) => {
        console.warn("Bookmarks onSnapshot network error (offline fallback):", error);
        const cachedFallback = localStorage.getItem('archivio_cached_bookmarks_' + currentUser.uid);
        if (cachedFallback) {
          try {
            setBookmarkedPapers(JSON.parse(cachedFallback));
            setIsOfflineLoaded(true);
          } catch (_) {}
        }
        setLoading(false);
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 md:px-8 py-8">
        ...
        {!currentUser ? (
          ...
          <div className="flex flex-col gap-4 w-full">
            {[1, 2, 3].map(n => (
              <div key={n} className="bg-[#fdfbf7] dark:bg-gray-800 border border-stone-200 dark:border-gray-700 shadow-sm flex flex-col md:flex-row justify-between p-6 gap-6 rounded w-full">
                <div className="flex-1 font-sans">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="h-5 w-24 rounded animate-shimmer"></div>
                    <div className="h-4 w-20 rounded animate-shimmer"></div>
                  </div>
                  <div className="h-6 w-3/4 rounded mb-2 animate-shimmer"></div>
                  <div className="h-4 w-1/2 rounded mb-4 animate-shimmer"></div>
                ...
                <div className="flex flex-col items-end justify-center md:w-36 flex-shrink-0 gap-3 w-full">
                  <div className="h-8 w-full rounded animate-shimmer"></div>
                  <div className="h-8 w-full rounded animate-shimmer"></div>
                </div>
              </div>
            ))}
