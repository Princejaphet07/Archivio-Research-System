...
export default function ChatWidget() {
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      ...
      const q = query(collection(db, 'groups'), where('leaderUid', '==', uid));
      ...
      <div className="flex-1 bg-[#FDF9ED] p-4 overflow-y-auto flex flex-col gap-3">
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
            ...
            <p className="text-sm font-semibold text-stone-700">Say hello to your adviser!</p>
            <p className="text-xs text-stone-500 mt-1">This is your group chat room.</p>
          </div>
        ) : (
          messages.map((msg, i) => {
