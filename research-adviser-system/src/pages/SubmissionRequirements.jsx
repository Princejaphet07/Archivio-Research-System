      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-bold text-gray-900 text-lg">Requirement Checklist</h3>
              <p className="text-xs text-gray-500">Items students must submit for completion</p>
            </div>
            <button 
              onClick={() => setShowModal(true)}
              className="bg-[#7a2e46] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#5f2135] transition"
            >
              + Add Requirement
            </button>
          </div>
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <p className="text-gray-500 text-sm py-4 text-center">No requirements found.</p>
            ) : (
              requirements.map((req) => (
                <div key={req.id} className="flex items-center justify-between border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition">
                  ...
                  <div className="flex items-center gap-3">
                    {/* Status Badge */}
                    {req.scope === 'global' ? (
                      <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider border border-blue-200">Global</span>
                    ) : req.status === 'approved' ? (
                      <span className="bg-green-50 text-green-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider border border-green-200">Approved</span>
                    ) : req.status === 'declined' ? (
                      <span className="bg-red-50 text-red-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider border border-red-200">Declined</span>
                    ) : (
                      <span className="bg-yellow-50 text-yellow-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider border border-yellow-200">Pending</span>
                    )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-md w-full shadow-2xl overflow-hidden">
            <div className="bg-[#7a2e46] p-4 text-white">
            ...
            <form onSubmit={handleAddRequirement} className="p-5 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:border-[#7a2e46]" 
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-100">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 border border-gray-300 text-gray-600 rounded-lg py-2 text-sm font-bold hover:bg-gray-50">Cancel</button>
                <button type="submit" className="flex-1 bg-[#7a2e46] text-white rounded-lg py-2 text-sm font-bold hover:bg-[#5f2135]">Propose Requirement</button>
              </div>
            </form>
          </div>
        </div>
